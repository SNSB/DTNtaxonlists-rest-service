...
# also change in the search.py 701, 704, 1137, 855, 1143, 1144, 849, 1140, 1129, 853, 852, 851, 1154, 923, 924, 925, 926, 927
...
def getTaxonNameLists(databasename):
    ...
    query = u'''select distinct a.ProjectID, a.ProjectURI from [%s].[dbo].[TaxonNameListProjectProxy] a \
                ...
                a.ProjectID in (701, 704, 1137, 855, 1143, 1144, 849, 1140, 1129, 853, 852, 851, 1154, 923, 924, 925, 926, 927) \
...
def getTaxonNameListsProjectUri(databasename, id):
    ...
    if not cleanDatabasename(databasename):
        return []
    databasename=diversitydatabase(databasename)
    query = u'select distinct ProjectURI from [%s].[dbo].[TaxonNameListProjectProxy] where ProjectID=%s and ProjectID in (701, 704, 1137, 855, 1143, 1144, 849, 1140, 1129, 853, 852, 851, 1154, 923, 924, 925, 926, 927)' % (databasename, id)
    current_app.logger.debug("Query %s " % (query))
    with get_db().connect() as conn:
        projectURI = conn.execute(query)
        if projectURI != None:
...
def getAllTaxonNamesFromList(databasename, listid):
    ...
    query = u'''select distinct a.NameID, a.Notes from [%s].[dbo].[TaxonNameList] a inner join [%s].[dbo].[TaxonName] b on a.NameID=b.NameID where \
                ...
                and ProjectID=%s and \
                ProjectID in (701, 704, 1137, 855, 1143, 1144, 849, 1140, 1129, 853, 852, 851, 1154, 923, 924, 925, 926, 927)''' % (databasename, databasename, listid)
    current_app.logger.debug("Query %s " % (query))
    with get_db().connect() as conn:
...
def getAllTaxonNamesFromListFlat(databasename, listid):
    ...
    query = u'''select '%s' as DatabaseName, a.NameID, a.TaxonNameCache, a.Version, a.TaxonomicRank as TaxonomicRankCode, c.DisplayText as TaxonomicRank, a.GenusOrSupragenericName, \
                ...
                and b.ProjectID in (701, 704, 1137, 855, 1143, 1144, 849, 1140, 1129, 853, 852, 851, 1154, 923, 924, 925, 926, 927) ''' % (databasename, databasename, databasename, databasename, databasename, databasename, databasename, listid)
    current_app.logger.debug("Query %s " % (query))
    with get_db().connect() as conn:
...
def getAllCommonNamesFromListFlat(databasename, listid):
    ...
    query = u'''select '%s' as DatabaseName, a.NameID, c.CommonName, c.CountryCode, c.LanguageCode, b.ProjectID \
                ...
                (a.IgnoreButKeepForReference is Null or a.IgnoreButKeepForReference=0) and \
                (a.DataWithholdingReason is Null or a.DataWithholdingReason='') \
                and b.ProjectID in (701, 704, 1137, 855, 1143, 1144, 849, 1140, 1129, 853, 852, 851, 1154, 923, 924, 925, 926, 927) ''' % (databasename, databasename, databasename, databasename, listid)
    current_app.logger.debug("Query %s " % (query))
    with get_db().connect() as conn:
        namelistproxy = conn.execute(query)
        if namelistproxy != None:
...
def getTaxonName(databasename, nameid):
    ...
    query = u'''select '%s' as DatabaseName, a.NameID, a.TaxonNameCache, a.Version, a.TaxonomicRank as TaxonomicRankCode, c.DisplayText as TaxonomicRank, a.GenusOrSupragenericName, \
                ...
                (a.IgnoreButKeepForReference is Null or a.IgnoreButKeepForReference=0) and \
                (a.DataWithholdingReason is Null or a.DataWithholdingReason='') \
                and ProjectID in (701, 704, 1137, 855, 1143, 1144, 849, 1140, 1129, 853, 852, 851, 1154, 923, 924, 925, 926, 927) ''' % (databasename, databasename, databasename, databasename, nameid)
    current_app.logger.debug("Query %s " % (query))
    with get_db().connect() as conn:
        namelistproxy = conn.execute(query)
        if namelistproxy != None:
